...
void main() {
    ...
    if (NdotL > 0.0) {
        specular_dir = vec3(pow(HdotN, 4.0)) * specularStrength;
    }
...
// --- CALCOLO OMBRA CON PCF (Soft Shadows) ---
    ...
    for(int i = 0; i < gubo.numLights; i++) {
        ...
        float verticalCutoff = clamp(1.0 - (yDistance / 4.0), 0.0, 1.0);
        ...
        if (NdotL_pt > 0.0) {
            specular_pt = vec3(pow(HdotN_pt, 2.0)) * specularStrength;
        }
    ...
    // ==========================================
    vec3 ambient = vec3(0.015, 0.018, 0.02) * albedo;
